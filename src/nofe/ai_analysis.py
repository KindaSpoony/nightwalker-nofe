...
FALLBACK_MODELS: Tuple[str, ...] = ("gpt-5.1-mini", "gpt-4.1", "gpt-4.1-mini")
REASONING_NONE_PREFIXES: Tuple[str, ...] = ("gpt-5.1",)
...
def _chat_new_client(
    ...
    if any(lower_model.startswith(prefix) for prefix in REASONING_NONE_PREFIXES):
...
def _normalize_model_name(model: Optional[str]) -> str:
    ...
    if lower == "gpt-5.1":
        return "gpt-5.1"
    if lower == "gpt-5.1-mini":
        return "gpt-5.1-mini"
    if lower == "gpt-4.1":
        return "gpt-4.1"
    if lower == "gpt-4.1-mini":
        return "gpt-4.1-mini"
    return normalized
